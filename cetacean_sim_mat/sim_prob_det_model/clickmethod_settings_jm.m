...
%if 1 then the points in the simulation are spaced evenly in x and y and
%thus range is triangular distriubtion. If 0 then the ranges are evenly
%distributed instead of even distributiuon in x and y. 
p.evenxy=0; 

p.maxrange = 750; %  the max expected detection range cutoff in meters
...
p.snr_thresh = 10; % Minimum RL cutoff for detectability, in dB
...
p.det_efficiency=prob_dist('Custom', {0, 110, dafaultdetefficiency}); 
p.perfect_detector=false; 


%% the vertical angle distribution in DEGREES
...
%% beam profile.
[ Xq,Yq,Vq, beamraw] = create_beam_profile(species, 'aperture', 0.0645/2);
p.horzBeam=Xq;
p.vertBeam=Yq;
p.tLBeam=Vq;
p.beamraw=beamraw;
