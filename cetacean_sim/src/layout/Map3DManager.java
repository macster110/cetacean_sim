...
public class Map3DManager  {
	...
	public void updateProviders() {
		...
		for (int i=0; i<mapProviders.size(); i++){
			for (int j=0; j<cetSimView.getSimUnits().size(); j++){
				if (cetSimView.getSimUnits().get(j).getMapShapeProvider()==mapProviders.get(i)){
					kk=true; 
				}
			}
		...
		for (int i=0; i<cetSimView.getSimUnits().size(); i++){
			System.out.println("Sim unit: "+cetSimView.getSimUnits().get(i));
			if (cetSimView.getSimUnits().get(i)==null) continue; 
			if (mapProviders.contains(cetSimView.getSimUnits().get(i).getMapShapeProvider())){
				//no need to re add model
				continue;
			}
			else {
				//add map provider
				addMapProvider(cetSimView.getSimUnits().get(i).getMapShapeProvider()); 
			}
