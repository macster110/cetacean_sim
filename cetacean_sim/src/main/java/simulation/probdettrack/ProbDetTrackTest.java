...
//		String filename = "/Users/au671271/Desktop/hp17_135a_tagdata_example.mat";
		...
		String filename = "/Users/jdjm/Desktop/hp18_134a_tagdata_example_no_move.mat";
		...
		ArrayList<RecievedInfo[][]> detTimeSeries = probDetTrack.getTrackDetTimeSeries(); 
		System.out.println("Recieved info time series length: " + detTimeSeries.size() + " " + detTimeSeries.get(0).length );
		
		
		
		SurfaceData beamSurface = SurfaceUtils.generateSurface(animalModel.getBeamProfileTL().getRawBeamMeasurments());
	
		
		double TLahead = beamSurface.grid.interpolate((float) Math.toRadians(0), (float)Math.toRadians(0));
		double TLbehind = beamSurface.grid.interpolate((float) Math.toRadians(180), (float) Math.toRadians(0)); 
		double TLleft = beamSurface.grid.interpolate((float) Math.toRadians(-40), (float) Math.toRadians(90)); 
		double TLRight = beamSurface.grid.interpolate((float) Math.toRadians(40), (float) Math.toRadians(0)); 

		System.out.println("TL ahead: " + TLahead + " TL behind: " + TLbehind + " TL left: " + TLleft + " TL right: " + TLRight);
