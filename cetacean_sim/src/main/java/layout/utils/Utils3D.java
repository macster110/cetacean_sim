...
public class Utils3D {
	...
	/**
	 * Convert a beam profile raw measurements to degrees from radians 
	 * @param array - the beam array [horz angle (rad), vert angle (rad), ...]
	 * @return float array with equivalent values 
	 */
	public static double[][] beam2Degrees(double[][] beamarray) {

		if (beamarray==null || beamarray.length==0) return null; 

		double[][] doubleArray = new double[beamarray.length][beamarray[0].length];

		for (int i=0; i<beamarray.length; i++) {
			for (int j=0; j<beamarray[0].length; j++) {
				if (j<2) {
					doubleArray[i][j]=Math.toDegrees(beamarray[i][j]); 
				}
				else {
					doubleArray[i][j]=beamarray[i][j]; 
				}
			}
		}
	...
	/**
	 * Convert a beam profile raw measurements to degrees from radians 
	 * @param array - the beam array [horz angle (degrees), vert angle (degrees), ...]
	 * @return float array with equivalent values 
	 */
	public static double[][] beam2Radians(double[][] beamarray) {
		...
		for (int i=0; i<beamarray.length; i++) {
			for (int j=0; j<beamarray[0].length; j++) {
				if (j<2) {
					doubleArray[i][j]=Math.toRadians(beamarray[i][j]); 
				}
				else {
					doubleArray[i][j]=beamarray[i][j]; 
				}
