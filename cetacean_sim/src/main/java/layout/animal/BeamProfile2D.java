...
public class BeamProfile2D extends BorderPane  {
	 ...
	 * Colour array 
	 */
	public Palette colourArray= new Palette(1000, Palette.DEFAULT_COLOR_PALETTE);
	...
	public BeamProfile2D(){
		// Wrap the canvas in a plain Pane. The Pane's size is controlled by the
		// BorderPane layout. The canvas is unmanaged so it does not influence the
		// Pane's preferred size, breaking the circular feedback loop that caused
		...
		canvasHolder.setPrefWidth(300);
		canvasHolder.setPrefHeight(300);
		...
		hbox.prefWidthProperty().bind(canvasHolder.widthProperty());
