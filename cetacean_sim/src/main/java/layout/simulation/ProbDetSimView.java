...
//				SimpleOdontocete.SIM_UNIFORM_DEPTH_HORZ, probDetSim.getProbDetSettings());
	...
	private Pane createSimControlPane() {
		...
		export = new Button(); 
		export.setGraphic(new MaterialIconView(MaterialIcon.SAVE, "25"));
	...
	private void setPlayButtonGraphic(boolean running) {
		Platform.runLater(()->{
			if (running) {
				play.setGraphic(new MaterialIconView(MaterialIcon.PAUSE, "25"));
			}
			else {
				play.setGraphic(new MaterialIconView(MaterialIcon.PLAY_ARROW, "25"));
			}
		});
	}
