...
public class SimpleDetectionPane extends BorderPane implements SettingsPane<Detector>{
	...
	private Pane createSimpleDetectionPane(){
		...
		graphButton.setGraphic(new FontAwesomeIconView(FontAwesomeIcon.LINE_CHART, "18"));
	...
	private void openDistGraph() {
		...
		if (chartDialog==null) {
			...
			dPane.getStylesheets().add(getClass().getResource("/darktheme.css").toExternalForm());
